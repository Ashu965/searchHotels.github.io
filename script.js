$(document).ready(function () {
    ...
    function select() {
     ...
     /* fetchData = async () => {
        const url =  "https://developers.zomato.com/api/v2.1/search?entity_id=" +
        valueDropdown +
        "&entity_type=city" +
        searchCity +
        "&count=100"
        let data = await fetch(url, {
          method: "GET",
          headers: { "user-key": "c2dc61d8b3b17609f7ddc3068c45ff1b" }
        });
        data = await data.json();
        console.log(data);
      };
      
      var settings =fetchData();*/
      var settings = {
        async: true,
        crossDomain: true,
        url:
      ...
      $.getJSON(settings, function (data) {
        ...
        $.each(data, function (index, value) {
          ...
          $.each(x, function (index, value) {
            ...
            html +=
              "<span title='" +
              userRating.rating_text +
              "'><p style='color:white;background-color:#" +
              userRating.rating_color +
            ...
            html += "</div>";
            html +=
              "<img class='resimg img-rounded' src=" +
              value.thumb +
              " alt='Restaurant Image' height='185' width='185'>";
            html +=
              "<a href=" +
              value.url +
              " target='_blank' class='action_link'><h2 style='color:red;'><strong>" +
              value.name +
              "</strong></h2></a>";
            html +=
